...
function designDataTable(rows){
  let body = rows.map((r,i)=>{
    ...
    return `<tr>
      ...
      <td>${particular}</td>
      <td><input type="text" data-field="${nid()}" value="${dDefault||''}" ${dDefault==='NA'?'placeholder="NA"':''}></td>
      <td><input type="text" data-field="${nid()}" ${cDefault==='NA'?'placeholder="NA"':''} value="${cDefault==='NA'?'NA':''}"></td>
      <td><input type="text" data-field="${nid()}" placeholder="${remarkDefault||''}"></td>
    </tr>`;
  }).join('');
...
function genTable(headers, nRows, selectCols={}){
  ...
  for(let i=0;i<nRows;i++){
    let tds = headers.map((h,ci)=>{
      ...
      if(selectCols[ci]) return `<td><input type="text" data-field="${nid()}" placeholder="${selectCols[ci].join(' / ')}"></td>`;
      return `<td><input type="text" data-field="${nid()}"></td>`;
    }).join('');
    rows += `<tr>${tds}</tr>`;
  }
...
ch3 += row(field('Full Reservoir Level (m)', {autofill:'Full Reservoir Level(m)', id:'reservoirLevelInput'}), field('Minimum Draw Down Level (m)'));
...
{
  let rows = instrumentList.map((name,i)=>`<tr><td style="text-align:center">${i+1}</td><td>${name}</td><td style="text-align:center"><input type="text" data-field="${nid()}" placeholder="Y/N" style="text-align:center"></td><td><input type="text" data-field="${nid()}"></td><td><input type="text" data-field="${nid()}"></td></tr>`).join('');
  ch3 += `<table class="data"><thead><tr><th>Sl.No</th><th>Name of Instrument</th><th style="width:60px">Working Status</th><th>Year of Installation</th><th>No. of Years data available</th></tr></thead><tbody>${rows}</tbody></table>`;
}
...
ch3 += field('Dates of site inspection, reservoir level &amp; conditions, members present, methods/equipment used, and any limitations', {type:'textarea'});
...
ch3 += field('Data gaps — essential documents unavailable and their implications', {type:'textarea', value:'Detail Drawings, Seismic Reports, and Design Flood Review'});
...
{
  const obligations = [
    ['Dam Safety Unit constituted within O&amp;M establishment (Sec. 30)', 'Complied', 'DSU Formed'],
    ['Pre-monsoon, post-monsoon and special inspections carried out (Sec. 31)', 'Complied', ''],
    ['Minimum instrumentation installed and readings analysed (Sec. 32)', 'Partly Complied', ''],
    ['Hydro-meteorological station established (Sec. 33)', 'Not Complied', ''],
    ['Seismological station established, where applicable (Sec. 34)', 'Not Complied', ''],
    ['Inflow forecasting &amp; emergency flood-warning system; periodic risk assessment (Sec. 35)', 'Not Complied', ''],
    ['Emergency Action Plan prepared and periodically updated (Sec. 36)', 'Complied', ''],
    ['O&amp;M establishment, trained staff and O&amp;M manual maintained (Sec. 28)', 'Complied', '']
  ];
  let rows = obligations.map(([o,status,remark],i)=>`<tr><td style="text-align:center">${i+1}</td><td>${o}</td><td style="width:130px"><input type="text" data-field="${nid()}" placeholder="Complied / Partly / Not" value="${status}"></td><td><input type="text" data-field="${nid()}" value="${remark}"></td></tr>`).join('');
  ch11 += `<table class="data"><thead><tr><th>Sl.No</th><th>Statutory Obligation</th><th>Status</th><th>Remarks</th></tr></thead><tbody>${rows}</tbody></table>`;
}
...
ch12 += note('Select all applicable statements from the dropdown below (repeat to add more).');
ch12 += selectAddField('Detailed safety and design evaluation — summary', ch12Options);
ch12 += field('Additional remarks / details, if any', {type:'textarea'});
addChapter(12, 'Detailed Safety and Design Evaluation', ch12);

/* ---------- CHAPTER 13 ---------- */
let ch13 = statictext('IPoE, based on CDSE outcome of the dam, shall provide the final classification of Dam Safety with rational justification.');
{
  const cats = [
    ['I','Unsafe; immediate action required'],
    ['II','Conditionally safe; remedial measures required'],
    ['III','Safe for continued operation']
  ];
  let rows = cats.map(([cat,label])=>`<tr><td style="text-align:center"><b>${cat}</b></td><td><label style="font-weight:400"><input type="radio" name="safetyCategory" data-field="${nid()}" data-safety-cat="${cat}" onchange="onSafetyCategoryChange(this)"${cat==='III'?' checked':''}> ${label}</label></td><td><input type="text" data-field="${nid()}" placeholder="Justification"></td><td><input type="text" data-field="${nid()}" placeholder="Remark"></td></tr>`).join('');
  ch13 += `<table class="data"><thead><tr><th style="width:40px">Category</th><th>Categorisation for Dam Operation</th><th>Justification</th><th>Remark</th></tr></thead><tbody>${rows}</tbody></table>`;
}
...
ch14 += selectAddField('Standard recommendations', [
  'Comply with the Dam Safety Act 2021, its Rules/Regulations, and the NDSA CDSE SOP.',
  'SDSO to ensure all recommended investigations/studies are completed on time, with periodic compliance review.',
  'Carry out detailed material testing and geotechnical, geological and non-destructive investigations.',
  'Update the stability analysis using the latest codes and field/lab investigation data.',
  'Reassess the Design Flood as per the latest hydrological studies and design criteria.',
  'Prepare/update the O&M Manual, EAP and Dam Safety Surveillance Manual per CWC/NDSA guidelines.',
  'Install adequate instrumentation, including GTS-linked benchmarks for deformation/settlement monitoring.',
      ...
      headers:{default: new Header({children:[ new Table({
        ...
        rows:[new TableRow({children:[new TableCell({
          ...
          children:[
            ...
            new Paragraph({children:[new TextRun({text:'Government of India · National Dam Safety Authority · Policy & Research Wing', color:'B7C4DA', size:14, font:FONT_SANS})]})
          ]
        })]})]
      })]})},
...
const DRIVE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwApVA6miFsZW7PRdoPwLMWeuUI_xPuttAPGFpGcE6HLjwBlyNXQolLDLDAoywjYjx-KQ/exec"
...
async function submitToDrive(overwrite){
  ...
  const damEl = document.getElementById('coverDamInput');
  const damName = (damEl && damEl.value) || 'Unknown_Dam';
  btn.disabled = true;
...
async function loadFromDriveByPic(){
  ...
  try{
    ...
    console.error(err);
    status.style.color = '#b23b2e';
    status.textContent = '⚠ Load fail hua — internet check karo, dubara try karo';
  } finally {
    btn.disabled = false;
