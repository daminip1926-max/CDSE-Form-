...
ch3 += row(field('Date of Starting Construction', {type:'date'}), field('Year of Commission', {autofill:'Year of Commission'}));
...
ch3 += row(field('Date of First Full Impoundment (MM/YYYY)'), field('Pre &amp; Post Monsoon Inspection carried out? (Y/N)', {type: 'textarea', 
    value: 'Yes'}));
...
ch3 += row(
  field('O&amp;M Manual — Year of Publication', { 
    type: 'textarea', 
    value: '2026' 
  }), 
  field('Emergency Action Plan — Year of Publication',{ 
    type: 'textarea', 
    value: '2026' 
  })
);
...
ch3 += field('Summary on adequacy and justification for additional instrumentation', {type:'textarea', value:'Instrumentation as per regulations may be installed'});
...
ch3 += field('Data gaps — essential documents unavailable and their implications', {type:'textarea', value:'Drawings were not available in the required form and shall be recast/revised as per the directions of NDSA. The available technical documentation was also incomplete, including design memoranda, design-flood and seismic studies, geotechnical reports, grouting records, and instrumentation records. These documents shall be compiled/reconstructed, wherever required, and updated to support the dam safety assessment and future operation, maintenance, and surveillance of the dam.'});
...
ch8 += selectAddField('Structural, geotechnical &amp; seepage safety — findings', ch8Options, 'The structural safety assessment is satisfactory. It includes structural and geotechnical evaluation under appropriate loading combinations, including static, seismic, thermal, and combined loading conditions. For gravity dams, the assessment covers sliding and overturning stability, foundation contact pressures, and stress distribution. For embankment dams, it includes slope stability under steady-state seepage and rapid drawdown conditions, identification of critical slip surfaces, pore-pressure assessment, and evaluation of filters, transitions, and internal stability. For arch dams, the assessment covers radial and tangential stresses, thrust-line behaviour, and abutment stability. The foundation assessment includes bearing capacity, settlement, uplift pressures, and the effectiveness of foundation drainage.');
...
const LABEL_TEXT_COLORS = {
    'Structural, geotechnical & seepage safety — findings': red
};
function getFieldLabelText(el){
  const wrap = el.closest('.field');
  const labelEl = wrap ? wrap.querySelector('label') : null;
  if(!labelEl) return '';
  return labelEl.textContent.replace(/\s+/g,' ').replace(/\s*auto\s*$/i,'').trim();
}
...
function colorFilledFieldsPurple() {
  document.querySelectorAll('input[data-field], textarea[data-field], select[data-field]').forEach(el => {
    ...
    // Agar box mein kuch likha hai, AUR usme "Not available" nahi likha hai, tabhi color do
    if(val !== '' && !val.includes('Not available')) {
      const labelText = getFieldLabelText(el);
      el.style.color = LABEL_TEXT_COLORS[labelText] || 'purple';
    } else {
